...
class BrainSphere(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        raw_Y1 = raw_Y1['data'][:,2]   # 0: curv, 1: sulc, 2: thickness
...
model = Generator(1, conv_type, pooling_type)
...
def train_step(data, target):
    ...
    loss = mseloss(torch.transpose(prediction,0,1), target) + 5 * l1Loss(torch.transpose(prediction,0,1), target)
