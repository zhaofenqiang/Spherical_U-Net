...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  3 21:07:15 2018
...
for f in range(5):
    ...
    #    train_Y0[i,:] = np.concatenate((feats_Y0_sulc, feats_Y0_thick), 0)
        ...
        feats_Y1 = raw_Y1['data'][:,2]   # 0: curv, 1: sulc, 2: thickness
        train_Y1[k,:] = feats_Y1
    ...
    train_Y0 = np.reshape(train_Y0, (len(train_files)*40962, 102))
    val_Y0 = np.reshape(val_Y0, (len(val_files)*40962, 102))
    train_Y1 = train_Y1.flatten()
    val_Y1 = val_Y1.flatten()
    
    mlp = neural_network.MLPRegressor(hidden_layer_sizes=(102,1000,1000,1), learning_rate_init=0.0001, max_iter=100)
    mlp.fit(train_Y0, train_Y1)
    val_thick_Y1_pred = mlp.predict(val_Y1)
    
    mae = np.absolute(val_Y1 - val_thick_Y1_pred)
    print("mlp: mae mean:", np.mean(mae))
    print("mlp: mae std:", np.std(mae))
    mre = np.absolute(val_Y1 - val_thick_Y1_pred)/val_Y1
    print("mlp: mre mean: ", np.mean(mre))
    print("mlp: mre std: ", np.std(mre))    
    
    if val_fold == 1:
        for i in range(len(val_files)):
            file = val_files[i]
            np.savetxt('./pred/mlp_pred_' + file.split('/')[-1][0:-3] + '_thickness' + '.txt', val_thick_Y1_pred[i,:])
