...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 14 21:03:50 2018
...
class BrainSphere(torch.utils.data.Dataset):
    ...
    def __init__(self, root1, root2 = None, root3 = None, root4 = None):
        ...
        if root2 is not None:
            self.files = self.files + sorted(glob.glob(os.path.join(root2,'*')))
        if root3 is not None:
            self.files = self.files + sorted(glob.glob(os.path.join(root3,'*')))
        if root4 is not None:
            self.files = self.files + sorted(glob.glob(os.path.join(root4,'*')))
...
#
    if epoch % 2 == 0:
       # a = val_train_during_training()
        #print("train Dice:", a)
        torch.save(model.state_dict(), os.path.join("age.pkl"))
#  
