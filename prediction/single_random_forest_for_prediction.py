...
for f in range(5):
    ...
    regr.fit(train_Y0, train_Y1)
    
    val_thick_Y1_pred = regr.predict(val_Y0)
    val_thick_Y1_pred = np.reshape(val_thick_Y1_pred, (20,40962))
        
    mae = np.absolute(val_Y1 - val_thick_Y1_pred)
    print("CRF: mae mean:", np.mean(mae))
    print("CRF: mae std:", np.std(mae))
    mre = np.absolute(val_Y1 - val_thick_Y1_pred)/val_Y1
    print("CRF: mre mean: ", np.mean(mre))
    print("CRF: mre std: ", np.std(mre))    
    
    if val_fold == 1:
        for i in range(len(20)):
            file = val_files[i]
            np.savetxt('./pred/CRF_pred_' + file.split('/')[-1][0:-3] + '_thickness' + '.txt', val_thick_Y1_pred[i,:])
